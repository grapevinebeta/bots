...
methods._parseRating = function($, data) {
    ...
    var html = $(data);
    doc.score = html.find("#coreRating img").attr("src").match(/([0-9\.]+).png/)[0];
    doc.score = this.float(doc.score);


    doc.count = this.int(html.find("#rdrwslnk strong").text().replace(/Reviews?/, ''));
...
methods._parseComments = function($, data, page, callback, scope) {
    ...
    html.find("div.reviewContainer:first .hreview").parent().each(function() {
        if (!self.more())return;
        $el = $(this);
...
methods._hasMore = function($, data, page) {
    ...
    return false;
    var last = $("#pagingdiv a:last", data);
