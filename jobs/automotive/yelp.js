...
methods._parseRating = function($, data) {
    ...
    doc.score = this.float($("#bizRating .star-img img").attr("title").replace(/[^0-9\.]+/g, ''));

    doc.count = this.int($("#bizRating .review-count .count").text());
...
methods._hasMore = function($, data, page) {
    var anchor = $("#paginationControls table a:last");
